...
class DPODataCollatorWithPadding:
    ...
    def tokenize_batch_element(
        ...
        if not self.is_encoder_decoder:
            ...
            referece_tokens = self.tokenizer(reference, add_special_tokens=False)
            ...
            eos_indices_reference = [i for i, x in enumerate(referece_tokens["input_ids"]) if x == eos_token_id]
            new_attention_mask_ref = [
                0 if i in eos_indices_reference else p for i, p in enumerate(referece_tokens["attention_mask"])
                ]
            referece_tokens["attention_mask"] = new_attention_mask_ref
            ...
            referece_tokens["input_ids"].append(self.tokenizer.eos_token_id)
            referece_tokens["attention_mask"].append(1)

            longer_response_length = max(len(chosen_tokens["input_ids"]), len(rejected_tokens["input_ids"]), len(referece_tokens["input_ids"]))
            ...
            reference_sequence_tokens = {k: prompt_tokens[k] + referece_tokens[k] for k in referece_tokens}
            ...
            reference_sequence_tokens["labels"][: len(prompt_tokens["input_ids"])] = [self.label_pad_token_id] * len(
                prompt_tokens["input_ids"]
                )
            ...
            for k, toks in {
                ...
                "reference": reference_sequence_tokens,
            ...
            referece_tokens = self.tokenizer(
            ...
            batch["reference_labels"] = referece_tokens["input_ids"]
            ...
            if self.model is not None and hasattr(self.model, "prepare_decoder_input_ids_from_labels"):
                ...
                batch["reference_decoder_input_ids"] = self.model.prepare_decoder_input_ids_from_labels(
                    labels=batch["reference_labels"]
                )

        batch["prompt"] = prompt
        batch["chosen"] = prompt + chosen
        batch["rejected"] = prompt + rejected
        batch["reference"] = prompt + reference
        batch["chosen_response_only"] = chosen
        batch["rejected_response_only"] = rejected
        batch["reference_response_only"] = reference
    ...
    def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for feature in features:
            ...
            batch_element = self.tokenize_batch_element(prompt, chosen, rejected, reference)
